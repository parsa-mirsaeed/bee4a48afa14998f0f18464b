-- Fix #34 source-replacement ordering so the existing lifecycle provenance guard
-- never observes a new canonical source while stale active OCR is still bound to
-- the prior source. Historical OCR revision evidence remains append-only in
...
BEGIN
    ...
    IF existing_source IS NOT NULL THEN
        -- Retire stale active OCR before the canonical pointer moves. Its
        -- verified revision provenance has already been copied to the immutable
        -- provenance table by trg_record_knowledge_ocr_revision_provenance.
