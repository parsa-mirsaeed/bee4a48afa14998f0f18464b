//! Strict local broker for the two approved external AI capabilities.
//!
//! The gateway has no database, Supabase, Qdrant, or storage credentials. It
//! accepts authenticated school-scoped requests from the application, applies
//! quotas, concurrency limits, bounded retries, strict response validation, and
//! independent circuit breakers, then calls fixed provider origins. Provider
//! availability is deliberately excluded from gateway and core application health.
...
use axum::{
    ...
    http::{header::AUTHORIZATION, HeaderMap as AxumHeaderMap, StatusCode as AxumStatusCode},
    response::{IntoResponse, Response},
    routing::{get, post},
    Json, Router,
};
use reqwest::{
    header::HeaderMap as ProviderHeaderMap, redirect::Policy as RedirectPolicy,
    StatusCode as ProviderStatusCode, Url,
};
use serde::Serialize;
...
const APPROVED_OPENAI_BASE_URL: &str = "https://api.openai.com/v1/";
const APPROVED_LLM_BASE_URL: &str = "https://api.deepseek.com/v1/";
const APPROVED_LLM_MODEL: &str = "deepseek-chat";
const INTERNAL_TEI_BASE_URL: &str = "http://embedding:80/v1/";
...
impl ProviderConfig {
    fn endpoint(&self, relative_path: &str) -> Result<Url, GatewayStartupError> {
        self.base_url.join(relative_path).map_err(|error| {
            GatewayStartupError::InvalidConfig(format!(
                "Unable to build approved provider endpoint: {error}"
            ))
        })
    }
}
...
impl GatewayConfig {
    fn from_env() -> Result<Self, GatewayStartupError> {
        let mode = GatewayMode::parse(&env_value("AI_GATEWAY_MODE", "connected"))?;
        ...
        let embedding_provider = match mode {
            GatewayMode::Connected => ProviderConfig {
                base_url: require_exact_external_url(
                    &env_value("AI_EMBEDDING_BASE_URL", APPROVED_OPENAI_BASE_URL),
            ...
            GatewayMode::Offline => ProviderConfig {
                base_url: require_internal_tei_url(&env_value(
                    "AI_EMBEDDING_BASE_URL",
                    INTERNAL_TEI_BASE_URL,
                ))?,
                api_key: None,
                model: embedding_profile.model.to_string(),
            },
        ...
        let configured_llm_model = env_value("LLM_MODEL", APPROVED_LLM_MODEL);
        if configured_llm_model != APPROVED_LLM_MODEL {
            return Err(GatewayStartupError::InvalidConfig(format!(
                "LLM_MODEL must be exactly {APPROVED_LLM_MODEL}"
            )));
        }
        let llm_provider = (mode == GatewayMode::Connected).then(|| {
            Ok::<ProviderConfig, GatewayStartupError>(ProviderConfig {
                base_url: require_exact_external_url(
                    &env_value("AI_LLM_BASE_URL", APPROVED_LLM_BASE_URL),
                    APPROVED_LLM_BASE_URL,
                )?,
        ...
        Ok(Self {
            ...
            internal_token_hash: sha256_bytes(internal_token.as_bytes()),
            mode,
            embedding_profile,
            embedding_provider,
            llm_provider,
            ...
            max_prompt_chars: env_u64("AI_MAX_PROMPT_CHARS", 80_000, 1_000, 500_000)
                as usize,
            max_output_tokens: env_u64("AI_MAX_OUTPUT_TOKENS", 4_096, 128, 16_384)
                as u32,
        })
    }

    #[cfg(test)]
    fn for_test(provider_base_url: Url) -> Self {
        Self {
            listen_addr: "127.0.0.1:0".parse().unwrap(),
            internal_token_hash: sha256_bytes(b"test-internal-token-abcdefghijklmnopqrstuvwxyz"),
            mode: GatewayMode::Connected,
            embedding_profile: crate::services::embedding_profile::OPENAI_V1,
            embedding_provider: ProviderConfig {
                base_url: provider_base_url.clone(),
                api_key: Some("test-provider-key-abcdefghijklmnopqrstuvwxyz".to_string()),
                model: crate::services::embedding_profile::OPENAI_V1.model.to_string(),
            },
    ...
    K: Eq + Hash + Copy,
{
    ...
    async fn consume(&self, key: K, characters: u64) -> Result<(), GatewayHttpError> {
        let now = Instant::now();
...
impl CircuitBreaker {
    ...
    async fn before_call(&self) -> Result<(), GatewayHttpError> {
        ...
        match state.mode {
            ...
            CircuitMode::Open { until } if now < until => Err(GatewayHttpError::new(
                AxumStatusCode::SERVICE_UNAVAILABLE,
                "circuit_open",
                "AI service temporarily unavailable",
            )
            .with_retry_after(until.duration_since(now).as_secs().max(1))),
            CircuitMode::Open { .. } => {
                state.mode = CircuitMode::HalfOpen;
                Ok(())
            }
            CircuitMode::HalfOpen => Err(GatewayHttpError::new(
                AxumStatusCode::SERVICE_UNAVAILABLE,
                "circuit_open",
                "AI service temporarily unavailable",
            )
            .with_retry_after(self.open_for.as_secs().max(1))),
        }
    }
...
impl ProviderFailure {
    ...
    fn counts_toward_circuit(self) -> bool {
        !matches!(self, Self::Unconfigured | Self::Rejected(status) if status < 500)
    }

    fn into_http_error(self) -> GatewayHttpError {
        match self {
            Self::RateLimited(seconds) => GatewayHttpError::new(
                AxumStatusCode::TOO_MANY_REQUESTS,
                self.code(),
                "AI provider rate limit reached",
            )
            .with_retry_after(seconds.clamp(1, 30)),
            Self::InvalidResponse | Self::ResponseTooLarge | Self::Rejected(_) => {
            ...
            Self::Unconfigured | Self::Unavailable => GatewayHttpError::new(
                AxumStatusCode::SERVICE_UNAVAILABLE,
                self.code(),
                "AI service temporarily unavailable",
            )
            .with_retry_after(self.retry_after().unwrap_or(30)),
        }
    }
}
...
async fn embeddings(
    ...
    state.execute_embeddings(&context, request).await.map(Json)
}
...
async fn chat_completions(
    ...
    let context = authenticate(&headers, &state.config.internal_token_hash)?;
    state.execute_chat(&context, request).await.map(Json)
}
...
fn authenticate(
    ...
    if !constant_time_eq(&sha256_bytes(token.as_bytes()), expected_token_hash) {
...
fn validate_embedding_request(
    ...
    if request.model != config.embedding_provider.model {
        return Err(bad_request("model_mismatch", "Embedding model does not match the active profile"));
    }
    ...
    if request.input.iter().any(|value| value.trim().is_empty()) {
        return Err(bad_request("empty_input", "Embedding inputs must not be empty"));
    }
...
fn validate_chat_request(
    ...
    let provider = config.llm_provider.as_ref().ok_or_else(|| unavailable("llm_disabled"))?;
...
async fn forward_embeddings(
    ...
    let api_key = if state.config.mode == GatewayMode::Connected {
        Some(
            state
                .config
                .embedding_provider
    ...
    for attempt in 0..=state.config.max_retries {
        ...
        match outbound.send().await {
            Ok(response) if response.status().is_success() => {
                let body = bounded_provider_body(response, state.config.max_provider_response_bytes)
                    .await?;
            ...
            Ok(response) => {
                ...
                let failure = provider_status_failure(status, retry_after);
            ...
            Err(error) => {
                let failure = if error.is_timeout() || error.is_connect() {
...
async fn bounded_provider_body(
...
fn required_internal_token() -> Result<String, GatewayStartupError> {
    let value = env::var("AI_GATEWAY_INTERNAL_TOKEN").map_err(|_| {
        GatewayStartupError::InvalidConfig("AI_GATEWAY_INTERNAL_TOKEN must be configured".to_string())
    })?;
...
fn optional_secret(name: &str) -> Result<Option<String>, GatewayStartupError> {
    match env::var(name) {
        Ok(value) if value.trim().is_empty() => Ok(None),
        Ok(value) if value.len() < 24 || looks_like_placeholder(&value) => {
            Err(GatewayStartupError::InvalidConfig(format!(
                "{name} is configured with an unsafe value"
            )))
        }
        Ok(value) => Ok(Some(value)),
        Err(env::VarError::NotPresent) => Ok(None),
        Err(error) => Err(GatewayStartupError::InvalidConfig(format!(
...
fn require_exact_external_url(
    value: &str,
    approved: &'static str,
) -> Result<Url, GatewayStartupError> {
    let requested = normalized_provider_url(value)?;
    let approved_url = normalized_provider_url(approved)?;
...
fn require_internal_tei_url(value: &str) -> Result<Url, GatewayStartupError> {
    let requested = normalized_provider_url(value)?;
    if requested.as_str() != INTERNAL_TEI_BASE_URL
        || requested.scheme() != "http"
        || requested.host_str() != Some("embedding")
        || requested.port_or_known_default() != Some(80)
    {
        return Err(GatewayStartupError::InvalidConfig(format!(
            "Offline embedding URL must be exactly {INTERNAL_TEI_BASE_URL}"
        )));
    }
    Ok(requested)
}
...
fn bad_request(code: &'static str, message: &'static str) -> GatewayHttpError {
    GatewayHttpError::new(AxumStatusCode::BAD_REQUEST, code, message)
}

fn unavailable(code: &'static str) -> GatewayHttpError {
    GatewayHttpError::new(
        AxumStatusCode::SERVICE_UNAVAILABLE,
        code,
        "AI service temporarily unavailable",
...
mod tests {
    ...
    #[test]
    fn provider_origins_are_compile_time_fixed() {
        assert!(require_exact_external_url(APPROVED_OPENAI_BASE_URL, APPROVED_OPENAI_BASE_URL).is_ok());
        assert!(require_exact_external_url(
    ...
    #[test]
    fn connected_and_offline_modes_require_separate_profiles() {
        assert!(validate_mode_profile(GatewayMode::Connected, OPENAI_V1).is_ok());
        assert!(validate_mode_profile(GatewayMode::Offline, LOCAL_BGE_V1).is_ok());
    ...
    #[tokio::test]
    async fn quota_is_per_school_and_counts_prompt_characters() {
        ...
        limiter.consume(first, 10).await.unwrap();
        assert_eq!(limiter.consume(first, 1).await.unwrap_err().code, "quota_exceeded");
        limiter.consume(second, 10).await.unwrap();
    }

    #[tokio::test]
    async fn circuit_allows_only_one_recovery_probe() {
    ...
    #[test]
    fn embedding_response_requires_finite_exact_dimension_vectors() {
        ...
        validate_embedding_response(OPENAI_V1, &request, &response).unwrap();
    ...
    #[test]
    fn chat_request_and_response_are_model_and_role_bound() {
        let config = GatewayConfig::for_test(Url::parse("http://127.0.0.1:1/v1/").unwrap());
        let request = GatewayChatRequest {
            model: APPROVED_LLM_MODEL.to_string(),
            messages: vec![
                GatewayChatMessage {
                    role: "system".to_string(),
                    content: "fixed policy".to_string(),
                },
                GatewayChatMessage {
                    role: "user".to_string(),
                    content: "authorized context".to_string(),
                },
            ],
            max_tokens: 128,
            temperature: 0.2,
        ...
        validate_chat_request(&config, &request).unwrap();
        let provider = config.llm_provider.as_ref().unwrap();
        ...
        validate_chat_response(provider, &response).unwrap();
    }
    ...
    #[tokio::test]
    async fn provider_outage_opens_circuit_and_recovery_resumes() {
        ...
        .await;
        let state = GatewayState::new(GatewayConfig::for_test(provider)).unwrap();
        let first = state
            .execute_embeddings(&context(), embedding_request())
            .await;
        assert_eq!(first.unwrap_err().code, "ai_temporarily_unavailable");
        let blocked = state
            .execute_embeddings(&context(), embedding_request())
            .await;
        assert_eq!(blocked.unwrap_err().code, "circuit_open");
    ...
    async fn rate_limit_retries_are_bounded() {
        ...
        .await;
        let mut config = GatewayConfig::for_test(provider);
