...
pub async fn get_pending_requests(
    auth_token: String,
) -> Result<Vec<ProfileChangeRequestResponse>, ServerFnError> {
    #[cfg(feature = "server")]
    {
        use crate::app_state::extract_server_state;
        use crate::repositories::profile_change_request_repository::ProfileChangeRequestRepository;
        ...
        if user.role_name.to_string() != "SchoolManager" {
            return Err(ServerFnError::new(
                "Only School Managers can view pending requests",
            ));
        }
        ...
        let requests = pcr_repo
            .list_by_status(PcrStatus::Pending, 100, 0)
            .await
        ...
        for req in requests {
            ...
            if let Some(req_user) = user_repo.find_by_id(req.user_id).await.ok().flatten() {
                if req_user.school_id == user.school_id {
                    ...
                    let requested_by = user_repo
                        .find_by_id(req.requested_by)
                        .await
                        .ok()
                        .flatten()
...
pub async fn decide_profile_change(
    ...
    #[cfg(feature = "server")]
    {
        ...
        if user.role_name.to_string() != "SchoolManager" {
            return Err(ServerFnError::new(
                "Only School Managers can decide requests",
            ));
        }
        ...
        if target_user.school_id != user.school_id {
            return Err(ServerFnError::new(
                "Cannot decide request for another school",
            ));
        }

        // Update request status
        let decide_req = DecideProfileChangeRequestRequest { status };

        pcr_repo
            .decide(req_id, user_id, decide_req)
            .await
        ...
        if status == PcrStatus::Approved {
            ...
            user_repo
                .update_internal(request.user_id, update_req)
                .await
