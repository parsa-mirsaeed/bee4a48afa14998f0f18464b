...
use crate::models::{CreateClassSectionRequest, ClassSectionWithSubject};
...
pub async fn get_school_classes() -> Result<Vec<ClassSectionResponse>, ServerFnError> {
    #[cfg(feature = "server")]
    {
        ...
        let user_uuid = Uuid::parse_str(&user.id).map_err(|_| ServerFnError::new("Invalid User ID"))?;
        ...
        Ok(rows.into_iter().map(|r| ClassSectionResponse {
            id: r.id.to_string(),
            name: r.name,
            term: r.term,
...
pub async fn get_subjects() -> Result<Vec<crate::models::Subject>, ServerFnError> {
    #[cfg(feature = "server")]
    {
        ...
        repo.list_all().await.map_err(|e| ServerFnError::new(e.to_string()))
    }
    #[cfg(not(feature = "server"))]
    Ok(vec![])
}
...
pub async fn create_class_section(
    ...
    #[cfg(feature = "server")]
    {
        ...
        let subject_uuid = Uuid::parse_str(&subject_id).map_err(|_| ServerFnError::new("Invalid Subject ID"))?;
        
        let user_uuid = Uuid::parse_str(&user.id).map_err(|_| ServerFnError::new("Invalid User ID"))?;
        let user_repo = &state.services.user;
        let db_user = user_repo.find_by_id(crate::domain::UserId::from(user_uuid))
            .await
        ...
        repo.create(request).await.map_err(|e| ServerFnError::new(e.to_string()))?;
