...
use crate::domain::{UserId, SchoolId, RoleId};
...
fn enforce_school_manager(user: &crate::domain::UserInfo) -> Result<(), ServerFnError> {
    if user.role != "SchoolManager" {
        return Err(ServerFnError::new("Unauthorized: Requires School Manager role"));
    }
    Ok(())
}
...
pub async fn get_school_users(
    ...
    #[cfg(feature = "server")]
    {
        ...
        let Extension(auth_user): Extension<crate::domain::UserInfo> = extract().await
        ...
        let current_user = user_repo.find_with_role_by_id(current_user_id)
            .await
            .map_err(|e| ServerFnError::new(format!("User not found: {}", e)))?;

        // 4. Query users in same school with filters
        let users = user_repo.find_by_school_with_filters(
...
pub async fn get_available_roles() -> Result<Vec<RoleInfo>, ServerFnError> {
    #[cfg(feature = "server")]
    {
        ...
        Ok(rows.into_iter().map(|row| RoleInfo {
...
pub async fn deactivate_user(
    ...
    #[cfg(feature = "server")]
    {
        ...
        let target_user = user_repo.find_by_id_internal(target_user_id)
            .await
            .map_err(|e| ServerFnError::new(format!("User not found: {}", e)))?;

        // 4. Verify same school
        if target_user.school_id != current_user.school_id {
            return Err(ServerFnError::new("Cannot deactivate users from other schools"));
        }
        ...
        // 6. Deactivate
        user_repo.update_active_status(target_user_id, false)
            .await
...
pub async fn reactivate_user(
    ...
    #[cfg(feature = "server")]
    {
        ...
        if target_user.school_id != current_user.school_id {
            return Err(ServerFnError::new("Cannot reactivate users from other schools"));
        }

        user_repo.update_active_status(target_user_id, true)
            .await
...
pub async fn create_user(
    ...
    #[cfg(feature = "server")]
    {
        ...
        let supabase_service = crate::services::supabase_auth::SupabaseAdminService::new(state.supabase_config.clone());
        ...
        let supabase_user = supabase_service.create_user(
            &payload.email,
            &payload.password,
            user_metadata
        ).await.map_err(|e| ServerFnError::new(format!("Supabase creation failed: {}", e)))?;
        ...
        match payload.role.as_str() {
            "Teacher" => {
                let teacher_id = user_repo.create_teacher(new_user_id, current_user.school_id, payload.subject)
                    .await
                    .map_err(|e| ServerFnError::new(format!("Failed to create teacher record: {}", e)))?;
                ...
                if let Some(meta) = &payload.metadata {
                    if let Some(classes) = meta.get("assigned_class_ids").and_then(|v| v.as_array()) {
                        ...
                        if !class_ids.is_empty() {
                            user_repo.assign_classes_to_teacher(teacher_id, class_ids)
                                .await
                                .map_err(|e| ServerFnError::new(format!("Failed to assign classes: {}", e)))?;
                        }
                    }
                }
            },
            "Student" => {
                let parent_id = if let Some(pid) = payload.parent_id {
                    Some(UserId::from(Uuid::parse_str(&pid).map_err(|_| ServerFnError::new("Invalid parent ID"))?))
                } else {
                    None
                };
                
                user_repo.create_student(new_user_id, current_user.school_id, parent_id, payload.talent_profile_ref)
                    .await
                    .map_err(|e| ServerFnError::new(format!("Failed to create student record: {}", e)))?;
            },
            "Parent" => {
                if let Some(meta) = &payload.metadata {
                    if let Some(students) = meta.get("associated_students").and_then(|v| v.as_array()) {
                        ...
                        if !student_ids.is_empty() {
                            user_repo.link_students_to_parent(new_user_id, student_ids)
                                .await
                                .map_err(|e| ServerFnError::new(format!("Failed to link students: {}", e)))?;
                        }
                    }
                }
            },
...
pub async fn get_user_stats() -> Result<UserStats, ServerFnError> {
    #[cfg(feature = "server")]
    {
        ...
        let (student_count, teacher_count, parent_count) = user_repo.get_user_counts(current_user.school_id)
            .await
...
pub async fn update_user_details(
    ...
    #[cfg(feature = "server")]
    {
        ...
        let target_user = user_repo.find_by_id(target_user_id)
            .await
        ...
        user_repo.update_internal(target_user_id, update_request)
            .await
