...
async fn extract_user_with_full_rls(
) -> Result<(UserInfo, std::sync::Arc<sqlx::PgPool>), ServerFnError> {
    let Extension(user): Extension<UserInfo> = extract()
        .await
        .map_err(|_| ServerFnError::new("Unauthorized: No active session"))?;
    ...
    let school_id: Option<Uuid> =
        sqlx::query_scalar!(r#"SELECT school_id FROM users WHERE id = $1"#, user_uuid)
            .fetch_optional(&*pool)
            .await
    ...
    #[cfg(feature = "server")]
    {
        ...
        let assignments = rows
            ...
            .map(|row| {
                ...
                let grade = row.grade.map(|g| {
                    percentage_to_letter_grade(g.to_string().parse::<f64>().unwrap_or(0.0))
                });
