...
pub async fn login_handler(
    ...
    tracing::debug!(
        "Login handler: attempting login for {} (form: {})",
        req.email,
        is_form_submission
    );
    ...
    let pg: PasswordGrantResponse = resp.json().await.map_err(|e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
    ...
    let user_info = verify_and_get_user(&state, &pg.access_token)
        .await
...
async fn verify_and_get_user(state: &AppState, token: &str) -> Result<UserInfo, String> {
    ...
    let user_uuid =
        uuid::Uuid::parse_str(&user_id).map_err(|e| format!("Invalid user ID format: {}", e))?;
