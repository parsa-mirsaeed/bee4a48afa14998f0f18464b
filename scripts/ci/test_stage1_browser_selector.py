...
MODULE_PATH = pathlib.Path(__file__).with_name('stage1_browser_selector.py')
SPEC = importlib.util.spec_from_file_location('stage1_browser_selector', MODULE_PATH)
...
class BrowserSelectorTests(unittest.TestCase):
    ...
    def test_backend_auth_selects_auth_and_authorization(self):
        self.assertEqual(
            selector.select(['packages/api/src/middleware/auth.rs']),
            '@auth|@authorization',
        )

    def test_assignment_workflow_selects_workflow_truth(self):
        self.assertEqual(
            selector.select(['packages/web/src/views/role_based/teacher/assignments.rs']),
            '@workflow-truth',
        )
    ...
    def test_shell_navigation_stays_full_smoke(self):
        self.assertEqual(
            selector.select(['packages/web/src/views/role_based/components/sidebar.rs']),
            '@smoke',
        )

    def test_harness_change_stays_full_smoke(self):
        self.assertEqual(
            selector.select(['tests/e2e/playwright.config.ts']),
            '@smoke',
        )
