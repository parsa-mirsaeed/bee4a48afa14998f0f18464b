...
# verify_knowledge_security.sh runs transaction-scoped RLS verification first,
# which creates/configures this NOBYPASSRLS application role and its grants.
app_role="edutalent_app_ci"
if ! psql "${DATABASE_URL}" -Atqc "SELECT 1 FROM pg_roles WHERE rolname='${app_role}' AND NOT rolbypassrls;" | grep -qx 1; then
  echo "Expected configured NOBYPASSRLS role ${app_role}" >&2
  exit 1
fi
...
# than throwing, so prove the protected row remained unchanged instead.
...
# The singleton OCR row is retained but becomes stale because its source binding
# no longer matches the canonical source. It must not count as current OCR.
state="$(psql "${DATABASE_URL}" -At --field-separator='|' \
  -c "SELECT (SELECT count(*) FROM knowledge_source_files WHERE asset_id='${asset_id}'),(SELECT count(*) FROM knowledge_ocr_texts WHERE asset_id='${asset_id}'),(SELECT count(*) FROM knowledge_ocr_texts AS ocr JOIN knowledge_assets AS asset ON asset.id=ocr.asset_id JOIN knowledge_source_files AS source ON source.id=asset.current_source_file_id AND source.asset_id=asset.id WHERE ocr.asset_id='${asset_id}' AND ocr.source_file_id=source.id AND lower(ocr.source_sha256)=lower(source.sha256)),(SELECT count(*) FROM knowledge_ocr_texts WHERE asset_id='${asset_id}' AND source_file_id='${initial_source}'),(SELECT count(*) FROM knowledge_ocr_revision_provenance WHERE revision='${ocr_revision}'),(SELECT count(*) FROM knowledge_audit_logs WHERE target_id='${asset_id}' AND action='knowledge_asset.metadata_updated'),(SELECT count(*) FROM knowledge_audit_logs WHERE target_id='${asset_id}' AND action='knowledge_asset.source_replaced');")"
IFS='|' read -r source_count ocr_rows current_ocr stale_old_source provenance metadata_audits source_audits <<<"${state}"
[[ "${source_count}" == 2 && "${ocr_rows}" == 1 && "${current_ocr}" == 0 && "${stale_old_source}" == 1 && "${provenance}" == 1 && "${metadata_audits}" -ge 2 && "${source_audits}" == 1 ]] || { echo "Source/OCR/audit invariant failed: ${state}" >&2; exit 1; }
