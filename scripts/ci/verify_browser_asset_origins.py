...
PRIVATE_ORIGIN = re.compile(r"https?://(?:localhost|127\\.0\\.0\\.1|10\\.|172\\.(?:1[6-9]|2\\d|3[0-1])\\.|192\\.168\\.)")
SECRET = re.compile(
    r"(?i)(?:sk-[A-Za-z0-9_-]{12,}|(?:api[_-]?key|secret|private[_-]?key|service[_-]?role|access[_-]?token)\\s*[:=]\\s*['\"][^'\"]{12,})"
)
ALLOWED_DATA_ORIGINS = {"data:", "blob:"}
...
def main() -> int:
    ...
    for path in iter_sources():
        ...
        for match in REMOTE_ORIGIN.finditer(text):
            ...
            if origin.startswith(tuple(ALLOWED_DATA_ORIGINS)):
                continue
            violations.append(f"{path.relative_to(ROOT)}: external browser origin {origin}")
        for pattern in (PRIVATE_ORIGIN, SECRET):
            if pattern.search(text):
                violations.append(f"{path.relative_to(ROOT)}: prohibited private/secret material")
    ...
    print(f"Browser asset origin/secret verification passed for {len(iter_sources())} source files")
    return 0
