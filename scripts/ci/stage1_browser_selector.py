...
An unknown/ambiguous browser-sensitive path returns the complete @smoke tier.
...
def select(files: Iterable[str]) -> str:
    ...
    # Harness/policy changes must prove the whole PR smoke tier.
    if any(
        p.startswith('tests/e2e/')
        or p.startswith('scripts/ci/run_browser_')
        or p == 'scripts/ci/verify_browser_harness.sh'
        or p == 'scripts/ci/stage1_browser_selector.py'
        or p == 'scripts/ci/test_stage1_browser_selector.py'
        for p in paths
    ):
        return '@smoke'
    ...
    for path in paths:
        ...
        if path.startswith('packages/web/src/') and any(
            token in path for token in ('login', 'auth', 'session')
        ):
            tags.add('@auth')
            continue
        ...
        # Navigation/shell/form changes can affect multiple role journeys. Keep
        # the complete smoke tier until narrower tagged coverage exists.
        if path.startswith('packages/web/src/'):
            ambiguous = True
            continue

        # Manual ci:browser escalation or a future browser-sensitive path should
        # never result in a narrower accidental omission.
        ambiguous = True
